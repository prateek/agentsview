...
describe("supportsResume", () => {
  it("returns true for supported agents", () => {
    ...
    expect(supportsResume("claude-cowork")).toBe(true);
  ...
  it("returns false for unsupported agents", () => {
    expect(supportsResume("claude-desktop")).toBe(false);
...
describe("supportsTerminalResume", () => {
  it("returns true only for agents with terminal resume support", () => {
    expect(supportsTerminalResume("claude")).toBe(true);
    expect(supportsTerminalResume("cursor")).toBe(true);
    expect(supportsTerminalResume("claude-cowork")).toBe(false);
  });
});

describe("supportsDesktopResume", () => {
  it("returns true only for agents with Claude Desktop support", () => {
    expect(supportsDesktopResume("claude")).toBe(true);
    expect(supportsDesktopResume("claude-cowork")).toBe(true);
    expect(supportsDesktopResume("cursor")).toBe(false);
  });
});

describe("buildResumeCommand", () => {
  it("generates claude resume command", () => {
    expect(buildResumeCommand("claude", "abc-123-def")).toBe(
      "claude --resume abc-123-def",
    );
  });

  it("generates codex resume command", () => {
    expect(buildResumeCommand("codex", "codex:sess-1")).toBe(
      "codex resume sess-1",
    );
  });

  it("generates gemini resume command", () => {
    expect(buildResumeCommand("gemini", "gemini:sess-2")).toBe(
      "gemini --resume sess-2",
    );
  });
  ...
  it("returns null for cowork (desktop-only resume)", () => {
    expect(
      buildResumeCommand(
        "claude-cowork",
        "claude-cowork:local_8d81b324-060a-460c-bd68-ab042f55fc12",
      ),
    ).toBeNull();
  });

  it("generates opencode resume command", () => {
    expect(buildResumeCommand("opencode", "opencode:s3")).toBe(
      "opencode --session s3",
    );
  });
  ...
  it("strips agent prefix from compound IDs", () => {
    expect(buildResumeCommand("codex", "codex:my-session-id")).toBe(
  ...
  it("generates copilot resume command", () => {
    expect(
      buildResumeCommand(
        "copilot",
        "copilot:a108ddbe-acdb-42f4-a35e-6c2938bf038b",
      ),
...
describe("resolveResumeCommand", () => {
  it("prefers the backend-formatted command when present", () => {
    expect(
      resolveResumeCommand("cursor", "cursor:chat-7", {
        command: "cursor agent --resume chat-7",
        cwd: "/tmp/project",
      }),
    ).toBe("cd '/tmp/project' && cursor agent --resume chat-7");
  });

  it("falls back to the local command builder when needed", () => {
    expect(
      resolveResumeCommand("codex", "codex:sess-1", null),
